#!/home/lakhatib/miniconda3/envs/birdman/bin/python
#SBATCH --chdir=/home/lakhatib/Wisconsin_MARS/metagenomics/birdman/diagnosis/normal_dementia/species
#SBATCH --output=/home/lakhatib/Wisconsin_MARS/metagenomics/birdman/diagnosis/normal_dementia/species/slurm_out/%x.out
...
for inference_dir in glob.glob('/home/lakhatib/Wisconsin_MARS/metagenomics/birdman/diagnosis/normal_dementia/species/inferences/*'):

    FEAT_REGEX = re.compile("F\d{4}_(.*).nc")
    omic_ = inference_dir.split('/')[-1]
    outfile = "/home/lakhatib/Wisconsin_MARS/metagenomics/birdman/diagnosis/normal_dementia/species/inferences-results/%s.beta_var.tsv" % (omic_)
